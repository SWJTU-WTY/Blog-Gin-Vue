...
<template>
    ...
    <div v-if="this.$store.state.themeObj.user_start!=0" class="footBack footBackQian">
        <!-- <div class="footer-img" :style="{background:this.$store.state.themeObj.bottom_img?'url('+this.$store.state.themeObj.bottom_img+') no-repeat 50%':'url(static/img/Go.jpg) no-repeat 50%'}"> -->
        <!-- </div> -->
    ...
    <div v-else class="footBack footBackHui">
        <div class="footer-img" >
            <img :src="this.$store.state.themeObj.bottom_img?this.$store.state.themeObj.bottom_img:'static/img/Go.jpg'" alt="">
        </div>
        <div class="fcontainer">
            <p>
                博客已萌萌哒运行<span v-html='longTime'>{{longTime}}</span><span class="timeJump">(●'◡'●)ﾉ♥</span>
